...
async function getReviews(placeId: string) {
  if (!placeId) {
    console.log("No place_id provided for reviews");
    return [];
  }
...
function transformToProfileData(data: TradespersonData) {
  ...
  return {
    ...
    isClaimed: false, // Scraped listings are unclaimed until owner verifies
  };
}
...
export default async function TradespersonPage({
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: data.tier1_name, item: `https://costatrades.com/locations/${tier1}` },
      { "@type": "ListItem", position: 3, name: data.tier2_name, item: `https://costatrades.com/locations/${tier1}/${tier2}` },
      { "@type": "ListItem", position: 4, name: `${data.category_display}s`, item: `https://costatrades.com/locations/${tier1}/${tier2}/${tradetype}` },
      { "@type": "ListItem", position: 5, name: data.name, item: `https://costatrades.com/locations/${tier1}/${tier2}/${tradetype}/${tradeperson}` },
    ],
  };

  // LocalBusiness Schema for AEO - individual business profile
  const localBusinessSchema = {
    "@context": "https://schema.org",
    "@type": "LocalBusiness",
    "@id": `https://costatrades.com/locations/${tier1}/${tier2}/${tradetype}/${tradeperson}#business`,
    name: data.name,
    description: `${data.name} provides professional ${data.category_display.toLowerCase()} services in ${data.tier2_name}, ${data.tier1_name}. Contact for reliable and quality service.`,
    url: `https://costatrades.com/locations/${tier1}/${tier2}/${tradetype}/${tradeperson}`,
    ...(data.phone && { telephone: data.phone }),
    ...(data.website && { sameAs: [data.website, data.google_maps_url].filter(Boolean) }),
    address: {
      "@type": "PostalAddress",
      streetAddress: data.address || "",
      addressLocality: data.tier2_name,
      addressRegion: data.tier1_name,
      addressCountry: "ES",
    },
    geo: {
      "@type": "GeoCoordinates",
    ...
    ...(data.rating > 0 && {
      aggregateRating: {
        ...
        reviewCount: (data.reviews_count || 1).toString(),
        bestRating: "5",
        worstRating: "1",
      },
    }),
  ...
  const serviceSchema = {
    "@context": "https://schema.org",
    "@type": "Service",
    serviceType: data.category_display,
    provider: {
      "@type": "LocalBusiness",
      "@id": `https://costatrades.com/locations/${tier1}/${tier2}/${tradetype}/${tradeperson}#business`,
    },
    areaServed: {
      "@type": "Place",
      name: `${data.tier2_name}, ${data.tier1_name}`,
    },
    description: `Professional ${data.category_display.toLowerCase()} services by ${data.name} in ${data.tier2_name}, Costa del Sol.`,
  };

  // Combined schema graph
  const combinedSchema = {
    "@context": "https://schema.org",
    "@graph": [breadcrumbSchema, localBusinessSchema, serviceSchema],
  };
