...
export async function POST(request: NextRequest) {
  try {
    const quoteData = (await request.json()) as QuoteEmailData;

    // Validate required fields
    if (
      !quoteData.customer?.email ||
      !quoteData.customer?.firstName ||
      !quoteData.jobDetails?.description
    ) {
    ...
    const info = await transporter.sendMail({
      from:
        ...
        '"Costa del Sol TradeFinder" <noreply@tradefinder.com>',
      to: "jon@onrise.ai",
    ...
    return NextResponse.json({
      ...
      previewUrl: process.env.SMTP_HOST
        ? undefined
