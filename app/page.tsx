...
export default function Home() {
  ...
  const faqs = [
    ...
    { question: "What areas do you cover?", answer: "We cover the entire Costa del Sol region, from Sotogrande to Nerja, including major hubs like Marbella, Estepona, Mijas, Fuengirola, Benalmádena, Torremolinos, and Malaga City." },
  ...
  const popularSearches = [
    "Best Electricians in Marbella",
    "Reliable Pool Service Marbella",
    "Expert Roofers Torremolinos",
    "Top Rated Plumbers in Estepona",
    "Professional Gardeners Benalmádena",
    "Local Handyman Calahonda",
    "Emergency AC Repair Fuengirola",
    "24 Hour Locksmith Nerja",
    "Solar Panel Installers Estepona",
    "Trusted Builders in Mijas Costa",
    "Quality Painters Sotogrande",
    "Bathroom Renovation Experts Marbella",
  ];
  ...
  const organizationSchema = {
    ...
    logo: "https://costatrades.com/images/logo.webp",
  ...
  const serviceSchema = {
    "@context": "https://schema.org",
    "@type": "Service",
    serviceType: "Home Improvement Services Directory",
    provider: {
      "@type": "Organization",
      name: "CostaTrades",
    },
...
/>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          8. PRE-FOOTER: LOCATIONS + SEO LINKS
          ══════════════════════════════════════════════════════════════════════ */}
      <section className="bg-white border-t border-slate-200 pt-10 pb-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          {/* Locations Dropdown */}
          <button
          ...
          {locationsOpen && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-4 mb-10 px-2">
              {locations.map((loc) => (
                <Link key={loc.slug} href={`/locations/${loc.slug}`} className="text-slate-600 hover:text-[#0a1f44] transition-colors text-sm">
                  {loc.name}
                </Link>
              ))}
            </div>
          ...
          {/* Search Links Grid */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-x-8 gap-y-3 text-sm">
            {[0, 1, 2, 3].map((col) => (
              <div key={col} className="space-y-2">
                {popularSearches.slice(col * 3, col * 3 + 3).map((search) => (
                  <Link key={search} href="#" className="block text-slate-600 hover:text-[#0a1f44] transition-colors">
                    {search}
                  </Link>
                ))}
              </div>
